...
Extra Options: used by GUI version of palera1n
    --gui               Use palera1n on terminal
    --device-info       Grep device info and exit | used by GUI version
    --boot              Booting the device (jailbroken)
...
_dfuhelper() {
     # GUI version will deal with the rest
    if [ "$gui" = '1' ]; then
      if [[ "$1" = 0x801* && "$deviceid" != *"iPad"* ]]; then
        echo -en "iPhone"
        exit 0 # from here, GUI is the leader
      else
        echo -en "iPad"
        exit 0 # from here, GUI is the leader
      fi
...
# ============
if [ "$gui" == '1' ]; then
    # something else will check for dependencies
    echo -en ''
else
...
if [ "$gui" == '1' ]; then
  # it's not nessecery, GUI does this
  echo -en ''
  else
...
# grep device info used by the GUI version of palera1n
if [ "$device_info" = "1" ]; then
  while [ "$(get_device_mode)" = "none" ]; do
    sleep 1
  done
  if [ "$("$dir"/ideviceinfo)" = 'ERROR: No device found!' ]; then
    echo -en "ERROR: could not connect to device "
  fi
  if [ "$(get_device_mode)" = "normal" ]; then
    echo -en "$(_info normal ProductType) (iOS $(_info normal ProductVersion)) Device connected in $(get_device_mode) mode "
    echo -en "\nECID: $(_info normal UniqueDeviceID)"
  elif [ "$(get_device_mode)" = "recovery" ]; then
    echo " $(_info recovery PRODUCT) Connected in $(get_device_mode) mode  ($(_info recovery MODEL))"
  elif [ "$(get_device_mode)" = 'dfu' ]; then
    echo "Connected in $(get_device_mode)"
  else
    echo -en 'ERROR: something went wrong'
  fi
  exit 0
fi
...
if [ "$enter_recovery" = '1' ]; then
  ...
  if [ "$(get_device_mode)" = "recovery" ]; then
    ...
    echo "Waiting For Device To Be In Recovery Mode"
    _wait recovery
    echo "Device Successfully Entered Recovery Mode"
    exit 0
  fi

fi

# dfu helper GUI version
if [ "$gui" = "1" ] && [ "$dfuhelper" = "1" ]; then
  cpid=$(_info recovery CPID)
  _dfuhelper $cpid
  exit
  else
    echo "Device Did not enter DFU :("
    exit 1
fi
